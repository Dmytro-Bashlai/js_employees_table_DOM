...
form.addEventListener("submit", function(e) {
    ...
    const inputPosition = form.querySelector('input[name="position"]').value.trim();
    ...
    for (const field of form.children){
        if (field.matches("label")) {
            const normalizedfieldName = field.firstChild.data.toLowerCase().replace(/[^a-z]/g, "");
...
function addEmployee(inputName, inputValue, row) {
    ...
    for (const headCell of headCells){
        ...
        if (normalizedHeadCell === inputName) {
            const checkedInputValue = inputName === "salary" ? formatSalary(inputValue) : inputValue;
